...
-- Recreate with the correct logic
...
BEGIN
    ...
    -- IMPORTANT: Get daily rate from payroll rules
    ...
    -- If no user-specific daily rate found, use global rules
    IF v_daily_rate = 0 OR v_daily_rate IS NULL THEN
        ...
        RAISE NOTICE 'Using global daily_rate: %', v_daily_rate;
    ELSE
        RAISE NOTICE 'Using user-specific daily_rate: %', v_daily_rate;
    END IF;
