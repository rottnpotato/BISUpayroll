-- Enhanced calculate_payroll_for_period to handle percentage-based payroll rules
...
DECLARE
    ...
    -- Variables for percentage-based calculations
    v_base_for_percentage DECIMAL := 0;
BEGIN
    ...
    -- Base for percentage calculations (regular pay before allowances/bonuses)
    v_base_for_percentage := v_regular_pay;

    -- Get allowances from payroll rules (both fixed and percentage-based)
    SELECT COALESCE(SUM(
        CASE 
            WHEN pr."isPercentage" = true THEN (pr.amount / 100) * v_base_for_percentage
            ELSE pr.amount
        END
    ), 0) INTO v_allowances
    ...
    -- Get bonuses from payroll rules (both fixed and percentage-based)
    SELECT COALESCE(SUM(
        CASE 
            WHEN pr."isPercentage" = true THEN (pr.amount / 100) * v_base_for_percentage
            ELSE pr.amount
    ...
    -- GSIS: 9% of contribution base
    v_gsis := calculate_gsis_contribution(v_contribution_base, 9.0, 0, 999999);
    
    -- PhilHealth: 2.5% of contribution base, min 500, max 5000, salary range 10000-100000
    v_philhealth := calculate_philhealth_contribution(v_contribution_base, 2.5, 500, 5000, 10000, 100000);
    
    -- Pag-IBIG: 2% of contribution base, min 100, max 200, salary range 1000-5000
    ...
    -- Get loan deductions from payroll rules (both fixed and percentage-based)
    SELECT COALESCE(SUM(
        CASE 
            WHEN pr."isPercentage" = true THEN (pr.amount / 100) * v_gross_pay
            ELSE pr.amount
        END
    ...
    -- Get other deductions (both fixed and percentage-based)
    SELECT COALESCE(SUM(
        CASE 
            WHEN pr."isPercentage" = true THEN (pr.amount / 100) * v_gross_pay
            ELSE pr.amount
...
COMMENT ON FUNCTION calculate_payroll_for_period IS 'Calculates complete payroll for a single user within a pay period. Handles both fixed and percentage-based payroll rules from user and admin assignments.';
