-- =====================================================
-- FIX: Regular pay should be based on days worked × daily rate
-- Not hours worked × hourly rate
-- =====================================================
-- Issue: Employee worked 9 days but got paid for 88.94 hours
-- Expected: 9 days × ₱1,000 = ₱9,000
-- Was getting: 88.94 hrs × ₱125/hr = ₱11,117.50
-- 
-- In a daily rate system, employees are paid per day worked,
-- regardless of how many hours they work (up to 8 hrs).
-- Hours beyond 8 require overtime approval.
-- =====================================================
...
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            -- Only count up to daily hours as holiday hours
            v_holiday_hours := v_holiday_hours + LEAST(rec.hours, v_daily_hours);
        END IF;
    END LOOP;
    
    -- Calculate overtime pay from APPROVED overtime requests
    SELECT * INTO overtime_rec FROM calculate_overtime_pay_from_requests(p_user_id, p_pay_period_start, p_pay_period_end);
    v_overtime_hours := overtime_rec.total_hours;
    v_overtime_pay := overtime_rec.total_pay;
    ...
    -- =====================================================
    -- FIXED: Regular pay = days worked × daily rate
    -- NOT hours worked × hourly rate
    -- =====================================================
    ...
    -- Get mandatory contributions from payroll rules
    ...
    -- Get withholding tax from payroll rules
...
COMMENT ON FUNCTION calculate_payroll_for_period IS 'Calculates complete payroll with DAILY RATE SYSTEM: Regular pay = days_worked × daily_rate (NOT hours × hourly_rate). Overtime/overload require explicit approval.';
