-- Fix payroll calculation to match accurate employee calculation method
-- Key fixes:
-- 1. Include overtime pay in contribution base for government deductions
-- 2. Use period-based overtime calculation (optional - keeping daily for now)
-- 3. Ensure gross pay includes all earnings properly
...
DECLARE
    ...
    v_expected_total_hours DECIMAL := 0;
...
BEGIN
    ...
    SELECT COALESCE(SUM(CAST("hoursWorked" AS DECIMAL)), 0) INTO v_hours_worked
    FROM "attendance_records"
    WHERE "userId" = p_user_id
    AND date >= p_pay_period_start
    AND date <= p_pay_period_end
    ...
    -- Calculate overtime hours (period-based: only hours beyond expected total)
    -- This matches the employee calculation approach
    ...
    -- FIX: Calculate contribution base including overtime pay
    -- This matches the employee calculation: (basePay + overtimePay) for percentage rules
    v_contribution_base := v_regular_pay + v_overtime_pay + v_allowances;
    
    -- Calculate government contributions with proper base
...
COMMENT ON FUNCTION calculate_payroll_for_period IS 'Calculates complete payroll for a single user within a pay period. Fixed to match employee calculation: period-based overtime and overtime included in contribution base.';
