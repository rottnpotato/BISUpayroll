...
BEGIN
    ...
    IF v_user_salary_grade IS NOT NULL AND v_user_position IS NOT NULL THEN
        ...
        v_position_rank := CASE 
            WHEN v_user_position ~* '\sVI$' THEN 6
            WHEN v_user_position ~* '\sV$' THEN 5
            WHEN v_user_position ~* '\sIV$' THEN 4
            WHEN v_user_position ~* '\sIII$' THEN 3
            WHEN v_user_position ~* '\sII$' THEN 2
            WHEN v_user_position ~* '\sI$' THEN 1
            ELSE 1
        END;

        SELECT sg."dailyRate" INTO v_daily_rate
        FROM "salary_grades" sg
        WHERE sg.grade = v_user_salary_grade
          AND sg.step = v_user_salary_step
          AND sg.position = v_position_base
          AND sg.rank = v_position_rank
          AND sg."isActive" = true
        LIMIT 1;
    END IF;

    IF v_daily_rate IS NULL OR v_daily_rate = 0 THEN
        v_daily_rate := COALESCE(v_user_daily_rate, 0);
    END IF;
    IF v_daily_rate IS NULL OR v_daily_rate = 0 THEN
    ...
    v_monthly_base_income := v_daily_rate * 22;
    v_taxable_income := GREATEST(0, v_monthly_base_income - v_gsis - v_philhealth - v_pagibig - v_thirteenth_month - v_sil);
...
BEGIN
    v_monthly_base_income := p_daily_rate * 22;
    IF v_monthly_base_income <= 0 THEN RETURN 0; END IF;

    v_gross_annual_income := v_monthly_base_income * 12;
    ...
    IF v_net_taxable_income <= 0 THEN RETURN 0; END IF;
