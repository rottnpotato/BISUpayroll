...
export async function GET(request: NextRequest) {
  ...
  const endDateValue = endDate ? manilaEndOfDayUTC(endDate) : undefined
    ...
    const records = recordsRaw.map((r) => ({
      ...
      // Related records
      overloadRecords: (r as any).overloadRecords?.map((ol: any) => ({
        ...ol,
        startTime: ol.startTime.toISOString(),
        endTime: ol.endTime.toISOString(),
        hoursWorked: toNumber(ol.hoursWorked),
        hourlyRate: toNumber(ol.hourlyRate),
        totalAmount: toNumber(ol.totalAmount),
        approvedAt: ol.approvedAt ? ol.approvedAt.toISOString() : null,
        createdAt: ol.createdAt.toISOString(),
        updatedAt: ol.updatedAt.toISOString(),
      })) || [],
      overtimeRequests: (r as any).overtimeRequests?.map((ot: any) => ({
        ...ot,
        startTime: ot.startTime.toISOString(),
        endTime: ot.endTime.toISOString(),
        hoursWorked: toNumber(ot.hoursWorked),
        hourlyRate: toNumber(ot.hourlyRate),
        totalAmount: toNumber(ot.totalAmount),
        approvedAt: ot.approvedAt ? ot.approvedAt.toISOString() : null,
